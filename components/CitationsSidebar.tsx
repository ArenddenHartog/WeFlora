...
const CitationsSidebar: React.FC<CitationsSidebarProps> = ({ messages, onCitationClick, onSourceClick, highlightedFileName, onCitationHover }) => {
    ...
    const getIconForSource = (sourceName: string) => {
        if (sourceName.endsWith('.xlsx')) return <FileSheetIcon className="h-4 w-4 text-green-600" />;
        if (sourceName.endsWith('.pdf')) return <FilePdfIcon className="h-4 w-4 text-red-600" />;
        if (sourceName.endsWith('.json')) return <FileCodeIcon className="h-4 w-4 text-blue-600" />;
                ...
                {allCitations.length === 0 && webSources.length === 0 && mapSources.length === 0 ? (
                    ...
                    <>
                        ...
                        {(webSources.length > 0 || mapSources.length > 0) && (
                             <div className="mb-6">
                                <h3 className="text-sm font-semibold text-blue-600 uppercase mb-3 flex items-center gap-2">
                                ...
                                <ul className="space-y-2">
                                    {webSources.map((source, index) => (
                                        <li key={`web-${index}`}>
                                             <div 
                                                className="w-full text-left p-2 rounded-lg bg-blue-50/50 hover:bg-blue-50 border border-blue-100 transition-colors group cursor-pointer"
                                                onClick={() => handleWebSourceClick(source.messageId)}
                                            >
                                                <a href={source.uri} target="_blank" rel="noopener noreferrer" className="block">
                                                    <div className="text-xs text-blue-500 mb-1">Web Result</div>
                                                    <div className="font-medium text-slate-700 text-sm hover:underline decoration-blue-300">
                                                        {source.title}
                                                    </div>
                                                </a>
                                            </div>
                                    ...
                                    {mapSources.map((source, index) => (
                                        <li key={`map-${index}`}>
                                             <div 
                                                className="w-full text-left p-2 rounded-lg bg-green-50/50 hover:bg-green-50 border border-green-100 transition-colors group cursor-pointer"
                                                onClick={() => handleWebSourceClick(source.messageId)}
                                            >
                                                <a href={source.uri} target="_blank" rel="noopener noreferrer" className="block">
                                                    <div className="text-xs text-green-600 mb-1">Map Location</div>
                                                    <div className="font-medium text-slate-700 text-sm hover:underline decoration-green-300">
                                                        {source.title}
                                                    </div>
                                                </a>
                                            </div>
