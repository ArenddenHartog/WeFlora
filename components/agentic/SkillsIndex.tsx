    ...
    <PageShell
      ...
      actions={
        <>
          ...
          <Link to="/flows" className={btnSecondary}>
            View Flows
          </Link>
        </>
      }
    >
      ...
      {filtered.length === 0 ? (
        <div className="mt-8 rounded-xl border border-dashed border-slate-200 bg-slate-50/50 p-10 text-center">
          ...
          <p className="mt-4 text-sm font-semibold text-slate-700">No skills match your search</p>
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((profile) => (
            ...
            >
              ...
              <Link to={`/skills/${profile.id}`} className="mt-4 block">
              ...
              <div className="mt-auto pt-4">
                <Link to={`/sessions/new?intent=skill:${profile.id}`} className={btnSecondary}>
                  Run
                </Link>
              </div>
            </div>
