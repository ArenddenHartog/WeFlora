...
const clarifyingPayload: FloraGPTResponseEnvelope = {
  ...
  data: {
    questions: ['Which planting context?']
  }
};
const clarifyingMessage: ChatMessage = {
  id: 'msg-3',
  sender: 'ai',
  text: 'Which planting context?',
  floraGPT: clarifyingPayload
};
const clarifyingEncoded = encodeMessageForDb(clarifyingMessage, 'thread-3');
const clarifyingDecoded = decodeMessageFromDb({
  id: 'msg-3',
  sender: clarifyingEncoded.sender,
  text: clarifyingEncoded.text,
  floragpt_payload: clarifyingEncoded.floragpt_payload,
  citations: clarifyingEncoded.citations,
  context_snapshot: clarifyingEncoded.context_snapshot,
  grounding: clarifyingEncoded.grounding,
  suggested_actions: clarifyingEncoded.suggested_actions,
  created_at: clarifyingEncoded.created_at
});
assert.equal(clarifyingDecoded.floraGPT?.responseType, 'clarifying_questions');
assert.ok(Array.isArray(clarifyingDecoded.floraGPT?.data?.questions));
