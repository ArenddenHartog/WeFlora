...
4. Inspect the persisted message payload (Supabase or app state) and confirm:
   - `responseType="clarifying_questions"`
   - `data.questions` exists
   - `reasoning_summary` and `follow_ups` are absent
...
2. Re-open the same thread.
3. **Expected:** The structured message still renders (Structured v0.2 badge, reasoning, table, and citations are visible).

## 5) Worksheet preview flow
